...
public class OVRSkeleton : MonoBehaviour
{
	...
	public bool updateRotations= true;
	...
	[SerializeField]
	public bool _updateRootScale = false;
	[SerializeField]
	...
	private void Update()
	{
		...
		if (data.IsDataValid)
		{
			...
			{
				if (_bones[0].Transform != null)
				{
					if (_skeletonType == SkeletonType.HandLeft || _skeletonType == SkeletonType.HandRight)
					{
						_bones[0].Transform.localRotation = data.BoneRotations[0].FromFlippedXQuatf();

						if (_bones[0].Id == BoneId.Hand_WristRoot)
						{
							_bones[0].Transform.localRotation *= wristFixupRotation;
						}
					}
					else
					{
						_bones[0].Transform.localRotation = data.BoneRotations[0].FromFlippedZQuatf();
					}
				}
			}
